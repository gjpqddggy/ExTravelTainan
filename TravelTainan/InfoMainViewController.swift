...
class InfoMainViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        self.myTableView = UITableView(frame: CGRect(x: 0, y: 0, width: self.fullsize.width, height: self.fullsize.height), style: .grouped)
    ...
    @objc func goDatasouce(){
        let requestUrl = URL(string: "http://data.tainan.gov.tw")
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 0 {
            if indexPath.row == 0 {
                ...
                button.addTarget(self, action: #selector(InfoMainViewController.goDatasouce), for: .touchUpInside)
                button.setTitle("資料： 台南市政府資料開放平臺", for: .normal)
