...
class BaseMainViewController: UIViewController, CLLocationManagerDelegate, UITableViewDelegate, UITableViewDataSource, URLSessionDelegate, URLSessionDownloadDelegate {
    ...
    override func viewDidLoad() {
        ...
        myActivityIndicator.center = CGPoint(x: self.fullsize.width * 0.5, y: self.fullsize.height * 0.5)
    ...
    func addData(){
        // 取得前一次取得資料的日期
        let fetchDate = myUserDefaults.object(forKey: "\(fetchType)FetchDate") as? Int
        
        // 如果尚未取得資料 或 前一次取得資料已經超過設定天數
        // 便向遠端 API 取得資料
        let date = fetchDate ?? 0
        if self.todayDateInt - date > self.refreshDays {
            self.normalGet(self.strTargetID)
        } else {
    ...
    func addTable(_ file: String?){
        ...
        self.myTableView = UITableView(frame: CGRect(x: 0, y: 0, width: self.fullsize.width, height: self.fullsize.height), style: .plain)
...
// MARK: CLLocationManagerDelegate Methods
    ...
    func locationManager(_ manager: CLLocationManager, didChangeAuthorization status: CLAuthorizationStatus) {
        if (status == CLAuthorizationStatus.denied) {
            ...
            for type in ["landmark", "consume", "hotel"] {
...
// MARK: functional methods
    ...
    func jsonParse(_ url :URL) {
        do {
            ...
            let dict = try JSONSerialization.jsonObject(with: Data(contentsOf: url), options: JSONSerialization.ReadingOptions.allowFragments)
            //print(dict)
            let dataArr = dict as! [AnyObject]
    ...
    func fetchLatitudeAndLongitudeFromData(_ data: AnyObject) -> (latitude: Double, longitude: Double){
        ...
        if let num = data["latitude"] as? String {
            ...
            latitude = Double(num)!
        } else if let num = data["lat"] as? String {
            latitude = Double(num)!
        ...
        if let num = data["longitude"] as? String {
            ...
            longitude = Double(num)!
        } else if let num = data["long"] as? String {
            longitude = Double(num)!
    ...
    func refreshAPIData(){
        ...
        // 將有限數量資料依照與使用者距離重新排序
        self.fillIntoAPIDataForDistanceAndSort(apiData)
